...
//20170618: new control class - Gael
...
//vd($this->VALUE);
      if(is_array($arrOption)){
         ...
         {
            if($strGrouping != "") $innerHTML .= "<optGroup label=\"$strGrouping\">";
            //print_rr($Options);
...
class ZoriDateControl extends ZoriControl2
{
   public function __construct($proto){
      foreach($proto as $key => &$value){
         //vd($key); vd($value);
   ...
   public function render()
   {
      ...
      //$this->html->{"aria-describedby"} = "inputSuccess2Status3";
      //print_rr($this->html);
      $attr = self::renderAttributes($this->html, $enqoute);
      //vd($attr); die;
      $js = "
