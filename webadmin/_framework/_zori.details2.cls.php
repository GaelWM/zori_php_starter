...
//$this->Fields[$row->COLUMN_NAME]->Type = "varchar";
         ...
         {//20150317 - added datalist control type- pj/jj
            ...
            //print_rr($row); //die("knsdbn");
               ...
               $this->Fields[$row->COLUMN_NAME]->sql = "SELECT $row->COLUMN_NAME AS ControlValue,
                  $row->COLUMN_NAME AS ControlText FROM $table WHERE $row->COLUMN_NAME IS NOT NULL
                  OR $row->COLUMN_NAME != '' GROUP BY ControlText ORDER BY ControlText";

               $this->Fields[$row->COLUMN_NAME]->sqlGrouping = "LEFT(ControlText,1)";

               $htmlOption = null;

               if(strpos($row->COLUMN_NAME,'lst') === 0){
                  $this->Fields[$row->COLUMN_NAME]->jsValidate = "
