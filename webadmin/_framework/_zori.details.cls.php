...
class xxZoriDetails extends Zori
{
   ...
   public function __construct()
   {
      ...
      // if($_REQUEST[RETURN_URL] != ""){//20140717 - added RETURN_URL - maanie
   ...
   {
      global $db, $DATABASE_SETTINGS ,$SystemSettings ; //from system.php
      //ini
      $currentDB = $DATABASE_SETTINGS[$SystemSettings[SERVER_NAME]]->database;
      $xdb = new ZoriDatabase($table, $ID, null, 0);

      if($ID != 0)
         $this->ToolBar->Label .=": <strong class='textColour'>$ID</strong>";
      else
         $this->ToolBar->Label .=": <strong class='textColour'>New Record</strong>";
      //print_rr($xdb->Fields);
   ...
   public function renderControls()
   {
      ...
      {//print_rr($column);
         ...
         $this->Fields[$idxCol]->ControlHTML = ZoriControl::renderControl($column->Control->tag, $column->Control);
...
//vd($this->jsValidate );
         ...
         if($column->Control->type == "hidden")
         {
            ...
            {
               $Test = "<tr><td class='' colspan='100%'>$column->NewGroup</td></tr>";
            }
            else
            {
               $Test = "";
   ...
   protected function ContentBox()
   {
      return "
         <table border='0' cellpadding='2' cellspacing='1' width='100%' id='tblContent'>
            ...
            <tr>
               <td colspan='100%' style='background: transparent; padding: 0px;'>
