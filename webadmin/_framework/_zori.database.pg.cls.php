...
Class ZoriDatabase
{
   ...
   {
      global $POSTGRESQL, $BR;

      $this->POSTGRESQL = $POSTGRESQL;
      //print_rr($this->MYSQLI);
      ...
      if($strTable != "")
      {
         $rst = $this->doQuery("SELECT * FROM $strTable;");
...
//die;
   ...
   public function doQuery($sql, $overrideDebug="")
   {
      ...
      $this->Rows = 0;
      if($overrideDebug != 2)
         $rst = pg_query($this->POSTGRESQL, $sql) or die(mynl2br($sql) . "<br><br>" . mynl2br(pg_last_error($this->POSTGRESQL) ."<BR><BR><a href='?exeClearSort=true'>Clear Sort Parameters</a>"));
      else
         return mynl2br($sql);
      if(strpos(strtoupper(substr($sql,0,10)), "SELECT ") > -1)
         $this->Rows = $this->num_rows($rst);// or die(strtoupper($sql) . "<br>" . mysqli_error($this->MYSQLI));
      else
